...
def make_model_and_eval(l_model, l_x_train, l_x_test, l_y_train, l_y_test):
    ...
    # 構築したモデルで予測
    y_pred_train = model.predict(l_x_train)
    y_pred_test = model.predict(l_x_test)
    ...
    # TODO 下記は、多クラス分類の場合、エラーになる。
    tn_train, fp_train, fn_train, tp_train = confusion_matrix(l_y_train, y_pred_train).ravel()
    tn_test, fp_test, fn_test, tp_test = confusion_matrix(l_y_test, y_pred_test).ravel()
    ...
    l_importance = pd.DataFrame({'cols': x_train.columns, 'importance': model.feature_importances_})
    l_importance = l_importance.sort_values('importance', ascending=False)
    l_cols = pd.DataFrame({'x_cols': x_train.columns})
...
if __name__ == '__main__':
    ...
    target_output_dir_name = f'results_{tg_ymd}'
